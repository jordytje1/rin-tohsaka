	...
	async run(msg, { query }) {
		...
		if (user) {
			return msg.say(stripIndents`
				...
				Place \`<rin:phone:block:${user.id}>\` in this channel's topic
			`);
		}
		...
		return msg.say(stripIndents`
			...
			Just the channel: Place \`<rin:phone:block:${channel.id}>\` in this channel's topic
			The entire server: Place \`<rin:phone:block:${channel.guild.id}>\` in this channel's topic
		`);
	}
};
