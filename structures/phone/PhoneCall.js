...
module.exports = class PhoneCall {
	...
	async start() {
		if (this.adminCall) {
			...
			if (this.recipient.topic && this.recipient.topic.includes('<rin:phone:auto-accept>')) {
				await this.accept();
				return this;
			}
			if (this.originDM) {
	...
	async accept() {
		...
		if (this.originDM || (this.origin.topic && !this.origin.topic.includes('<rin:phone:no-notice>'))) {
		...
		if (this.originDM) {
			if (this.recipient.topic && !this.recipient.topic.includes('<rin:phone:no-notice>')) {
	...
	async hangup(nonQuitter, validation) {
		...
		if (nonQuitter === 'time') {
			...
			const canVoicemail = this.recipient.topic && !this.recipient.topic.includes('<rin:phone:no-voicemail>');
	...
	sendNotice(channel, dm) {
		...
		return channel.send(stripIndents`
			⚠️ **Notice:** ⚠️
			Sending illegal, NSFW, obscene, or hateful content can result in you and your server being banned from Rin.
			To report abuse, use ${reportUsage} (please include IDs and user tags).
			${dm ? '' : 'To hide this notice, place `<rin:phone:no-notice>` in your channel topic.'}
		`);
	}
};
