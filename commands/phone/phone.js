...
module.exports = class PhoneCommand extends Command {
	...
	async run(msg, { channelID }) {
		if (channelID !== 'count' && (msg.guild && (!msg.channel.topic || !msg.channel.topic.includes('<rin:phone>')))) {
			return msg.say('You can only start a call in a channel with `<rin:phone>` in the topic.');
		}
		...
		const channels = this.client.channels.cache.filter(channel => channel.guild
			&& channel.topic
			&& channel.topic.includes('<rin:phone>')
			&& !channel.topic.includes('<rin:phone:no-random>')
		...
		if (channelID) {
			...
			if (!channel.topic || !channel.topic.includes('<rin:phone>')) {
